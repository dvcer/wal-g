...
while [ ! -f "${WALG_MYSQL_BINLOG_DST}/mysql-bin.000001.ok" ]; do sleep 1; done

# wal-g should be blocked in REPLAY_COMMAND waiting for ${WALG_MYSQL_BINLOG_DST}/mysql-bin.000002.in
mysql -e "INSERT INTO sbtest.pitr VALUES('testpitr_last', NOW())"
mysql -e "FLUSH LOGS"
wal-g binlog-push
sleep 1

echo "proceed" > "${WALG_MYSQL_BINLOG_DST}/mysql-bin.000001.in"

for idx in 2 3 4 5 6 7 8 9
do
while [ ! -f "${WALG_MYSQL_BINLOG_DST}/mysql-bin.00000$idx.ok" ]; do sleep 1; done
echo "proceed" > "${WALG_MYSQL_BINLOG_DST}/mysql-bin.00000$idx.in"
done
